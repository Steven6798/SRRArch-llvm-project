...
static cl::opt<unsigned> SSThreshold(
    "srrarch-ssection-threshold", cl::Hidden,
    cl::desc("Small data and bss section threshold size (default=0)"),
    cl::init(0));

void SRRArchTargetObjectFile::Initialize(MCContext &Ctx,
...
// section.
bool SRRArchTargetObjectFile::isGlobalInSmallSection(
...
// Return true if this global address should be placed into small data/bss
// section.
bool SRRArchTargetObjectFile::isGlobalInSmallSection(const GlobalObject *GO,
                                                   const TargetMachine &TM,
...
// kind.
bool SRRArchTargetObjectFile::isGlobalInSmallSectionImpl(
    const GlobalObject *GO, const TargetMachine &TM) const {
...
MCSection *SRRArchTargetObjectFile::SelectSectionForGlobal(
...
bool SRRArchTargetObjectFile::isConstantInSmallSection(const DataLayout &DL,
                                                     const Constant *CN) const {
  return isInSmallSection(DL.getTypeAllocSize(CN->getType()));
}

MCSection *SRRArchTargetObjectFile::getSectionForConstant(
