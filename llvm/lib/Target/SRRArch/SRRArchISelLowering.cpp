...
static cl::opt<int> SRRArchLowerConstantMulThreshold(
    "srrarch-constant-mul-threshold", cl::Hidden,
...
SRRArchTargetLowering::SRRArchTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i32, &SRRArch::GPRRegClass);

  // Compute derived properties from the register classes
  TRI = STI.getRegisterInfo();
  computeRegisterProperties(TRI);

  setStackPointerRegisterToSaveRestore(SRRArch::SP);
...
Register SRRArchTargetLowering::getRegisterByName(
  ...
  Register Reg = StringSwitch<Register>(RegName)
                     ...
                     .Case("sp", SRRArch::SP)
                     .Case("fp", SRRArch::FP)
                     .Case("rr1", SRRArch::RR1)
                     .Case("r10", SRRArch::R10)
                     .Case("rr2", SRRArch::RR2)
                     .Case("r11", SRRArch::R11)
                     .Case("rca", SRRArch::RCA)
                     .Default(Register());
  return Reg;
}

std::pair<unsigned, const TargetRegisterClass *>
SRRArchTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &SRRArch::GPRRegClass);
    default:
      break;
    }
...
SRRArchTargetLowering::getSingleConstraintMatchWeight(
...
void SRRArchTargetLowering::LowerAsmOperandForConstraint(
...
static bool CC_SRRArch32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (!ArgFlags.isVarArg())
    return CC_SRRArch32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, OrigTy, State);
...
SDValue SRRArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // SRRArch target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue SRRArchTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  SRRArchMachineFunctionInfo *SRRArchMFI = MF.getInfo<SRRArchMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_SRRArch32_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_SRRArch32);
  }
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    Register Reg = SRRArchMFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      SRRArchMFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    SRRArchMFI->setVarArgsFrameIndex(FI);
  }
...
bool SRRArchTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, RetCC_SRRArch32);
}
...
SRRArchTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_SRRArch32);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, SRRArch::RV, Val, Glue);
    Glue = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(SRRArch::RV, getPointerTy(DAG.getDataLayout())));
  }
...
SDValue SRRArchTargetLowering::LowerCCCCallTo(
  ...
  if (IsVarArg) {
    CCInfo.AnalyzeCallOperands(Outs, CC_SRRArch32_VarArg);
  } else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_SRRArch32_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_SRRArch32);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, SRRArch::SP,
  ...
  uint8_t OpFlag = SRRArchII::MO_NO_FLAG;
  if (G) {
...
SDValue SRRArchTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_SRRArch32);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and SRRArch only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue SRRArchTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Glue = DAG.getNode(SRRArchISD::SET_FLAG, DL, MVT::Glue, LHS, RHS);

  return DAG.getNode(SRRArchISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Glue);
}

SDValue SRRArchTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > SRRArchLowerConstantMulThreshold)
    return SDValue();
...
SDValue SRRArchTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(SRRArchISD::SETCC, DL, Op.getValueType(), TargetCC, Glue);
}

SDValue SRRArchTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(SRRArchISD::SELECT_CC, DL, Op.getValueType(), TrueV, FalseV,
                     TargetCC, Glue);
}

SDValue SRRArchTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  SRRArchMachineFunctionInfo *FuncInfo = MF.getInfo<SRRArchMachineFunctionInfo>();
...
SDValue SRRArchTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For SRRArch, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(SRRArchISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue SRRArchTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue SRRArchTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, SRRArch::FP, VT);
  unsigned Depth = Op.getConstantOperandVal(0);
  while (Depth--) {
...
SDValue SRRArchTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const SRRArchTargetObjectFile *TLOF =
      static_cast<const SRRArchTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlign(), N->getOffset(), SRRArchII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(SRRArch::R0, MVT::i32),
                       DAG.getNode(SRRArchISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = SRRArchII::MO_ABS_HI;
    uint8_t OpFlagLo = SRRArchII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(SRRArchISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(SRRArchISD::LO, DL, MVT::i32, Lo);
...
SDValue SRRArchTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, SRRArchII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(SRRArch::R0, MVT::i32),
                       DAG.getNode(SRRArchISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue SRRArchTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue SRRArchTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), SRRArchII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(SRRArch::R0, MVT::i32),
                       DAG.getNode(SRRArchISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue SRRArchTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // SRRArch shifts of larger than register sizes are wrapped rather than
...
SDValue SRRArchTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of SRRArch's shift behavior to avoid needing to
...
SDValue SRRArchTargetLowering::PerformDAGCombine(SDNode *N,
...
void SRRArchTargetLowering::computeKnownBitsForTargetNode(
  ...
  default:
    break;
  case SRRArchISD::SETCC:
    Known = KnownBits(BitWidth);
    Known.Zero.setBits(1, BitWidth);
    break;
  case SRRArchISD::SELECT_CC:
    KnownBits Known2;
