...
static SRRArch::Fixups FixupKind(const MCExpr *Expr) {
  if (isa<MCSymbolRefExpr>(Expr))
    return SRRArch::FIXUP_SRRARCH_21;
  if (const MCSpecifierExpr *McExpr = dyn_cast<MCSpecifierExpr>(Expr)) {
    SRRArch::Specifier ExprKind = McExpr->getSpecifier();
    switch (ExprKind) {
    case SRRArch::S_None:
      return SRRArch::FIXUP_SRRARCH_21;
    case SRRArch::S_ABS_HI:
      return SRRArch::FIXUP_SRRARCH_HI16;
    case SRRArch::S_ABS_LO:
      return SRRArch::FIXUP_SRRARCH_LO16;
    }
  }
  return SRRArch::Fixups(0);
}
...
unsigned SRRArchMCCodeEmitter::getMachineOpValue(
    ...
    const MCSubtargetInfo &SubtargetInfo) const {
  if (MCOp.isReg())
    return getSRRArchRegisterNumbering(MCOp.getReg());
  if (MCOp.isImm())
...
static unsigned adjustPqBits(const MCInst &Inst, unsigned Value,
  ...
  if (!LPAC::isPostOp(AluCode) &&
      ((Op2.isImm() && Op2.getImm() != 0) ||
       (Op2.isReg() && Op2.getReg() != SRRArch::R0) || (Op2.isExpr())))
    Value |= (1 << PBitShift);
  ...
  if (LPAC::modifiesOp(AluCode) && ((Op2.isImm() && Op2.getImm() != 0) ||
                                    (Op2.isReg() && Op2.getReg() != SRRArch::R0)))
    Value |= (1 << QBitShift);
...
unsigned
SRRArchMCCodeEmitter::adjustPqBitsRmAndRrm(const MCInst &Inst, unsigned Value,
                                         const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 17, 16);
}

unsigned
SRRArchMCCodeEmitter::adjustPqBitsSpls(const MCInst &Inst, unsigned Value,
                                     const MCSubtargetInfo &STI) const {
...
// Encode SRRArch Memory Operand
unsigned SRRArchMCCodeEmitter::getRiMemoryOpValue(
  ...
  Encoding = (getSRRArchRegisterNumbering(Op1.getReg()) << 18);
  if (Op2.isImm()) {
...
unsigned SRRArchMCCodeEmitter::getRrMemoryOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  assert(Op1.isReg() && "First operand is not register.");
  Encoding = (getSRRArchRegisterNumbering(Op1.getReg()) << 15);
  assert(Op2.isReg() && "Second operand is not register.");
  Encoding |= (getSRRArchRegisterNumbering(Op2.getReg()) << 10);

  assert(AluMCOp.isImm() && "Third operator is not immediate.");
  // Set BBB
  unsigned AluOp = AluMCOp.getImm();
  Encoding |= LPAC::encodeSRRArchAluCode(AluOp) << 5;
  // Set P and Q
...
unsigned
SRRArchMCCodeEmitter::getSplsOpValue(const MCInst &Inst, unsigned OpNo,
                                   SmallVectorImpl<MCFixup> &Fixups,
                                   const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  Encoding = (getSRRArchRegisterNumbering(Op1.getReg()) << 12);
  if (Op2.isImm()) {
...
unsigned SRRArchMCCodeEmitter::getBranchTargetOpValue(
  ...
  Fixups.push_back(MCFixup::create(0, MCOp.getExpr(), SRRArch::FIXUP_SRRARCH_25));
