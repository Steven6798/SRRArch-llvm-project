...
// ADJDYNALLOC pseudo instructions with a SRRArch:ADDI with the
// maximum call frame size as the immediate.
void SRRArchFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const SRRArchInstrInfo &LII = *STI.getInstrInfo();
  unsigned MaxCallFrameSize = MF.getFrameInfo().getMaxCallFrameSize();

  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() == SRRArch::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(MBB, MI, DL, LII.get(SRRArch::ADD_I_LO), Dst)
            .addReg(Src)
...
void SRRArchFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(SRRArch::SW_RI))
      .addReg(SRRArch::FP)
      .addReg(SRRArch::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(SRRArch::ADD_I_LO), SRRArch::FP)
      .addReg(SRRArch::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(SRRArch::SUB_I_LO), SRRArch::SP)
        .addReg(SRRArch::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
void SRRArchFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
  ...
  BuildMI(MBB, MBBI, DL, LII.get(SRRArch::ADD_I_LO), SRRArch::SP)
      .addReg(SRRArch::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(SRRArch::LDW_RI), SRRArch::FP)
      .addReg(SRRArch::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}
