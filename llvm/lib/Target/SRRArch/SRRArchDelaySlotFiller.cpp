//===-- SRRArchDelaySlotFiller.cpp - SRRArch delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("srrarch-nop-delay-filler", cl::init(false),
                       cl::desc("Fill SRRArch delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "SRRArch Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const SRRArchSubtarget &Subtarget = MF.getSubtarget<SRRArchSubtarget>();
...
// createSRRArchDelaySlotFillerPass - Returns a pass that fills in delay
// slots in SRRArch MachineFunctions
FunctionPass *
llvm::createSRRArchDelaySlotFillerPass(const SRRArchTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == SRRArch::RET) {
        ...
        assert(RI->getOpcode() == SRRArch::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == SRRArch::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == SRRArch::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == SRRArch::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == SRRArch::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == SRRArch::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(SRRArch::NOP));
        }
        ++FilledSlots;
      }
