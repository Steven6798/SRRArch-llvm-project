...
// Helper functions for ComplexPattern used on SRRArchInstrInfo
// Used on SRRArch Load/Store instructions.
bool SRRArchDAGToDAGISel::selectAddrSls(SDValue Addr, SDValue &Offset) {
  ...
  if (Addr.getOpcode() == ISD::OR &&
      Addr.getOperand(1).getOpcode() == SRRArchISD::SMALL) {
...
bool SRRArchDAGToDAGISel::selectAddrRiSpls(SDValue Addr, SDValue &Base,
  ...
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(Addr)) {
    if (RiMode) {
      ...
      if (isInt<16>(CN->getSExtValue())) {
        ...
        Offset = CurDAG->getTargetConstant(Imm, DL, CN->getValueType(0));
        Base = CurDAG->getRegister(SRRArch::R0, CN->getValueType(0));
...
bool SRRArchDAGToDAGISel::selectAddrRi(SDValue Addr, SDValue &Base,
                                     SDValue &Offset, SDValue &AluOp) {
  return selectAddrRiSpls(Addr, Base, Offset, AluOp, /*RiMode=*/true);
}

bool SRRArchDAGToDAGISel::selectAddrSpls(SDValue Addr, SDValue &Base,
                                       SDValue &Offset, SDValue &AluOp) {
...
namespace llvm {
namespace LPAC {
static AluCode isdToSRRArchAluCode(ISD::NodeType Node_type) {
  switch (Node_type) {
  case ISD::ADD:
    return AluCode::ADD;
  case ISD::ADDE:
...
bool SRRArchDAGToDAGISel::selectAddrRr(SDValue Addr, SDValue &R1, SDValue &R2,
                                     SDValue &AluOp) {
  // if Address is FI, get the TargetFrameIndex.
  ...
  ISD::NodeType AluOperator = static_cast<ISD::NodeType>(Addr.getOpcode());
  LPAC::AluCode AluCode = LPAC::isdToSRRArchAluCode(AluOperator);
  if (AluCode != LPAC::UNKNOWN) {
    ...
    if (Addr.getOperand(0).getOpcode() == SRRArchISD::HI ||
        Addr.getOperand(0).getOpcode() == SRRArchISD::LO ||
        Addr.getOperand(0).getOpcode() == SRRArchISD::SMALL ||
        Addr.getOperand(1).getOpcode() == SRRArchISD::HI ||
        Addr.getOperand(1).getOpcode() == SRRArchISD::LO ||
        Addr.getOperand(1).getOpcode() == SRRArchISD::SMALL)
      return false;
