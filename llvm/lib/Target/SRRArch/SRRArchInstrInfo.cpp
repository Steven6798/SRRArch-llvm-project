...
void SRRArchInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                                 MachineBasicBlock::iterator Position,
                                 const DebugLoc &DL,
                                 Register DestinationRegister,
                                 Register SourceRegister, bool KillSource,
                                 bool RenamableDest, bool RenamableSrc) const {
  if (!SRRArch::GPRRegClass.contains(DestinationRegister, SourceRegister)) {
    llvm_unreachable("Impossible reg-to-reg copy");
  }

  BuildMI(MBB, Position, DL, get(SRRArch::OR_I_LO), DestinationRegister)
...
void SRRArchInstrInfo::storeRegToStackSlot(
  ...
  BuildMI(MBB, Position, DL, get(SRRArch::SW_RI))
...
void SRRArchInstrInfo::loadRegFromStackSlot(
  ...
  BuildMI(MBB, Position, DL, get(SRRArch::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}
...
SRRArchInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF, 0u);
}

ArrayRef<std::pair<unsigned, const char *>>
SRRArchInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  using namespace SRRArchII;
  static const std::pair<unsigned, const char *> TargetFlags[] = {
      {MO_ABS_HI, "srrarch-hi"},
      {MO_ABS_LO, "srrarch-lo"},
      {MO_NO_FLAG, "srrarch-nf"}};
...
bool SRRArchInstrInfo::analyzeCompare(const MachineInstr &MI, Register &SrcReg,
  ...
  default:
    break;
  case SRRArch::SFSUB_F_RI_LO:
  case SRRArch::SFSUB_F_RI_HI:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = Register();
    CmpMask = ~0;
    CmpValue = MI.getOperand(1).getImm();
    return true;
  case SRRArch::SFSUB_F_RR:
    SrcReg = MI.getOperand(0).getReg();
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == SRRArch::SFSUB_F_RR &&
      OI->getOpcode() == SRRArch::SUB_R &&
  ...
  if (((CmpI->getOpcode() == SRRArch::SFSUB_F_RI_LO &&
        OI->getOpcode() == SRRArch::SUB_I_LO) ||
       (CmpI->getOpcode() == SRRArch::SFSUB_F_RI_HI &&
        OI->getOpcode() == SRRArch::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case SRRArch::ADD_I_HI:
    return SRRArch::ADD_F_I_HI;
  case SRRArch::ADD_I_LO:
    return SRRArch::ADD_F_I_LO;
  case SRRArch::ADD_R:
    return SRRArch::ADD_F_R;
  case SRRArch::ADDC_I_HI:
    return SRRArch::ADDC_F_I_HI;
  case SRRArch::ADDC_I_LO:
    return SRRArch::ADDC_F_I_LO;
  case SRRArch::ADDC_R:
    return SRRArch::ADDC_F_R;
  case SRRArch::AND_I_HI:
    return SRRArch::AND_F_I_HI;
  case SRRArch::AND_I_LO:
    return SRRArch::AND_F_I_LO;
  case SRRArch::AND_R:
    return SRRArch::AND_F_R;
  case SRRArch::OR_I_HI:
    return SRRArch::OR_F_I_HI;
  case SRRArch::OR_I_LO:
    return SRRArch::OR_F_I_LO;
  case SRRArch::OR_R:
    return SRRArch::OR_F_R;
  case SRRArch::SL_I:
    return SRRArch::SL_F_I;
  case SRRArch::SRL_R:
    return SRRArch::SRL_F_R;
  case SRRArch::SA_I:
    return SRRArch::SA_F_I;
  case SRRArch::SRA_R:
    return SRRArch::SRA_F_R;
  case SRRArch::SUB_I_HI:
    return SRRArch::SUB_F_I_HI;
  case SRRArch::SUB_I_LO:
    return SRRArch::SUB_F_I_LO;
  case SRRArch::SUB_R:
    return SRRArch::SUB_F_R;
  case SRRArch::SUBB_I_HI:
    return SRRArch::SUBB_F_I_HI;
  case SRRArch::SUBB_I_LO:
    return SRRArch::SUBB_F_I_LO;
  case SRRArch::SUBB_R:
    return SRRArch::SUBB_F_R;
  case SRRArch::XOR_I_HI:
    return SRRArch::XOR_F_I_HI;
  case SRRArch::XOR_I_LO:
    return SRRArch::XOR_F_I_LO;
  case SRRArch::XOR_R:
    return SRRArch::XOR_F_R;
  default:
    return SRRArch::NOP;
  }
}
...
bool SRRArchInstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == SRRArch::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(SRRArch::SR, TRI) ||
        Instr.readsRegister(SRRArch::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != SRRArch::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(SRRArch::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != SRRArch::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool SRRArchInstrInfo::analyzeSelect(const MachineInstr &MI,
  ...
  assert(MI.getOpcode() == SRRArch::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
bool SRRArchInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == SRRArch::BT) {
      if (!AllowModify) {
...
unsigned SRRArchInstrInfo::insertBranch(MachineBasicBlock &MBB,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(SRRArch::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(SRRArch::BRCC)).addMBB(TrueBlock).addImm(ConditionalCode);
  ...
  BuildMI(&MBB, DL, get(SRRArch::BT)).addMBB(FalseBlock);
  return 2;
}
...
unsigned SRRArchInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != SRRArch::BT &&
        Instruction->getOpcode() != SRRArch::BRCC) {
      break;
    }
...
Register SRRArchInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
  ...
  if (MI.getOpcode() == SRRArch::LDW_RI)
